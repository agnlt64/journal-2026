...
import { projectSchema, ProjectFormValues, ProjectDTO, PROJECT_STATUS_LABELS } from "@/lib/types";
...
import { FolderGit, Plus, Link2, X, ExternalLink } from "lucide-react";
...
const STATUS_OPTIONS: { value: ProjectDTO["status"]; label: string; color: string }[] = [
    { value: "DRAFT", label: PROJECT_STATUS_LABELS["DRAFT"], color: "#888888" },
    { value: "IN_PROGRESS", label: PROJECT_STATUS_LABELS["IN_PROGRESS"], color: "#00f5ff" },
    { value: "POC_DONE", label: PROJECT_STATUS_LABELS["POC_DONE"], color: "#b829dd" },
    { value: "MVP_DONE", label: PROJECT_STATUS_LABELS["MVP_DONE"], color: "#ffbe0b" },
    { value: "DONE", label: PROJECT_STATUS_LABELS["DONE"], color: "#7bb88b" },
    { value: "ARCHIVED", label: PROJECT_STATUS_LABELS["ARCHIVED"], color: "#555555" },
];
...
export function ProjectDialog({ children, projectToEdit }: ProjectDialogProps) {
    ...
    const form = useForm<ProjectFormValues>({
        ...
        defaultValues: projectToEdit ? {
            ...
            links: projectToEdit.links.map(l => ({ id: l.id, title: l.title, url: l.url })),
        } : {
            title: "",
            description: "",
            status: "DRAFT",
            ...
            <DialogContent className={cn(
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pt-6">
                    ...
                    <div className="space-y-3">
                        <Label className="text-[10px] text-[rgba(123,184,139,0.6)] uppercase tracking-[0.2em]">Titre</Label>
                        <Input
                            {...register("title")}
                    ...
                    {/* Status */}
                    <div className="space-y-3">
                        <Label className="text-[10px] text-[rgba(123,184,139,0.6)] uppercase tracking-[0.2em]">Statut</Label>
                        <div className="flex flex-wrap gap-2">
                            {STATUS_OPTIONS.map((option) => (
                                <button
                                    ...
                                    style={{
                                        backgroundColor: currentStatus === option.value ? `${option.color}30` : "transparent",
                                        borderColor: currentStatus === option.value ? `${option.color}60` : undefined,
                                        boxShadow: currentStatus === option.value ? `0 0 15px ${option.color}30` : undefined,
                                    }}
                                >
                                    {option.label}
                                </button>
                    ...
                    {/* Description */}
                    <div className="space-y-3">
                        <Label className="text-[10px] text-[rgba(123,184,139,0.6)] uppercase tracking-[0.2em]">Description</Label>
                        <Textarea
                        ...
                        <div className="space-y-2">
                            ...
                            {fields.length === 0 && (
                                <p className="text-sm text-[rgba(255,255,255,0.3)] italic">
                                    Aucun lien ajouté. Cliquez sur &quot;Ajouter&quot; pour ajouter des ressources.
                                </p>
                            )}
                        </div>
                    </div>
