...
import { projectSchema, ProjectFormValues, ProjectDTO, ProjectLinkDTO } from "@/lib/types";
...
export async function getProjects(): Promise<ProjectDTO[]> {
    ...
    return projects.map((p) => ({
        ...
        links: p.links.map((l: ProjectLink): ProjectLinkDTO => ({
            id: l.id,
            title: l.title,
            url: l.url,
        })),
...
export async function updateProjectStatus(id: string, status: ProjectDTO["status"]) {
    const user = await getCurrentUser();
